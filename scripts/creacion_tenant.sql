...
hotel_stella AS (
  ...
  SELECT stella.tenant_id, 'Hotel Stella', 'Av. Principal 123', '+56911111111', 'contacto@stella.com' FROM stella
  RETURNING hotel_id, tenant_id
),
hotel_madero AS (
  INSERT INTO hotel (tenant_id, nombre, direccion, telefono, email)
  SELECT madero.tenant_id, 'Hotel Madero', 'Calle Secundaria 456', '+56922222222', 'contacto@madero.com' FROM madero
  RETURNING hotel_id, tenant_id
),
-- Crear usuarios
admin_stella AS (
