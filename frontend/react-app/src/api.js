const BASE = import.meta.env.VITE_API_URL || "http://backend:4000";
...
async function request(path, opts = {}) {
  ...
  if (!res.ok) {
    ...
    catch { return Promise.reject({ error: text || res.statusText }); }
  }
...
export const api = {
  ...
  // habitaciones endpoints (si no existen en backend, debes crearlos)
  getHabitaciones: () => request("/api/habitaciones", { method: "GET" }),
  updateHabitacion: (id, body) => request(`/api/habitaciones/${id}`, { method: "PUT", body: JSON.stringify(body) }),
};